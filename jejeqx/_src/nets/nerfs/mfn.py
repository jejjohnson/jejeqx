...
class MFNBase(eqx.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if depth == 0:
            layers.append(
                Linear(in_features=width_size, out_features=out_size, use_bias=use_bias, key=keys[0])
            )
        else:
            for i in range(depth - 1):
                layers.append(
                    Linear(in_features=width_size, out_features=width_size, use_bias=use_bias, key=keys[i + 1])
                )
            layers.append(
                Linear(in_features=width_size, out_features=out_size, use_bias=use_bias, key=keys[-1])
            )
        ...
        self.final_activation = Identity() if final_activation is None else final_activation
...
class FourierLayer(eqx.Module):
    ...
    def __init__(self,
        ...
        self.linear = Linear(in_features=in_features, out_features=out_features, use_bias=use_bias, key=key)
        
        # modify weights and biases
        self.linear = eqx.tree_at(lambda x: x.weight, self.linear, weight_scale * self.linear.weight)
    ...
    def __init__(
        ...
        *fkeys, key = jrandom.split(key, depth+2)
        super().__init__(
            ...
            depth=depth, w0=w0, use_bias=use_bias, final_activation=final_activation,
            key=key)
        ...
        for i in range(depth + 1):
            ...
            filters.append(
                FourierLayer(
                    in_features=in_size, out_features=width_size, use_bias=use_bias,
                    weight_scale=input_scale/jnp.sqrt(depth+1), key=fkeys[i]
                ),
            )
...
class GaborLayer(eqx.Module):
    ...
    def __init__(self,
        ...
        self.mu = 2 * jrandom.normal(key=gkeys[0], shape=(in_features, out_features)) - 1
        self.gamma = jrandom.gamma(key=gkeys[1], a=alpha, shape=(out_features,))
        
        self.linear = Linear(in_features=in_features, out_features=out_features, use_bias=use_bias, key=gkeys[2])
