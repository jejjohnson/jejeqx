...
class AlongTrackDM(pl.LightningDataModule):
    ...
    def preprocess(self):
        ...
        assert len(set(self.spatial_coords).intersection(column_names)) == len(self.spatial_coords)

        msg = f"No requested temporal coordinates found in dataset:"
        msg += f"\nTemporal Coords: {self.temporal_coords}"
        msg += f"\nColumns: {column_names}"
        assert len(set(self.temporal_coords).intersection(column_names)) == len(self.temporal_coords)
